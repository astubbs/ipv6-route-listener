...
def test_process_ra_with_prefix_and_route(
    packet_parser, route_configurator, mock_logger, mock_executor
):
    ...
    mock_logger.info.assert_any_call(
        f"🔧 Configuring prefix for {ra_data['prefix']['address']}/{ra_data['prefix']['length']}"
    )
    mock_logger.info.assert_any_call(
        f"🔧 Configuring route for {ra_data['route']['address']}/{ra_data['route']['length']}"
    )


def test_process_ra_with_non_ula_prefix(
    packet_parser, route_configurator, mock_logger, mock_executor
):
    ...
    # Configure the parser to return our test data
    packet_parser.parse = MagicMock(
        return_value={"src_ip": ra_data["src_ip"], "prefix": ra_data["prefix"]}
    )
